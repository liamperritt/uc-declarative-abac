...
The privileges, securables, governed-tags, **and policies** differs all follow this pattern — see the private `_resolve_*` function in each `<domain>/differ.py`. The policies differ resolves `to`/`except` principals per-principal too: a config-side (desired) failure stays fatal, while an actual-state (UC-side) failure is a non-fatal warning. Its one nuance: only the offending principal is dropped from the policy's principal list — **the policy itself is retained** (the other domains drop the whole row). No separate per-domain `resolver.py` modules exist.
...
`$ref: $defs/<type>/<key>` — inspired by JSON Schema's `$defs` and `$ref` keywords. The `<type>` is one of: `catalogs`, `schemas`, `tables`, `volumes`, `functions`, `policies`, `groups`. (The resolver is type-agnostic: it resolves `$defs/<type>/<key>` for any `<type>` present under `definitions:`.) A reference target inside a definition may itself contain `{{ placeholder }}` tokens (e.g. `$ref: $defs/tables/base_{{ layer }}`) — see Template variables below; the same applies to a bare inline `$defs/...` string value.
...
A definition may be a *template* containing `{{ placeholder }}` tokens; a `$ref` supplies values via a sibling `$vars` block (a definition is a function, a `$ref` is a call, `$vars` are the arguments). **A placeholder is bound by the `$vars` of the enclosing definition — the definition in whose text it appears (its body, or an override that definition writes onto a child `$ref`): the writer binds it.** Consequently a placeholder may live only inside a definition; one anywhere under a `resources:` entry (plain value, `$ref` override value, `$vars` value, or dict key) is an error — resources are concrete. Placeholders appear in string **values**, in **tag-name map keys** — the keys of `tags`, `has_tags`, `has_any_of_tags` (`TEMPLATABLE_KEY_FIELDS` in `configs/variables.py`), which are user data — and in a **`$ref`/`$defs` target** within a definition (e.g. `$ref: $defs/tables/base_{{ layer }}`, root or nested; also a bare inline `$defs/...` string), where the placeholder selects which definition is referenced and is substituted to a concrete target before lookup. Every other dict key stays literal (a placeholder there is a hard error at `finalise`). A templated target under `resources:` is still an error — resources carry no `$vars` scope. Because binding is local to the writer, a parent cannot widen a child's variable contract via an override, and a child's declared `$vars` signature is a closed contract. A definition may also declare its own `$vars` block for per-variable defaults (`name: value`) and/or a required-variable signature (`name:` / null); **a declared `$vars` block must be complete** (list exactly the placeholders the body uses — counting forwarded, child-`$ref`-override, `$ref`-target, and base-`$ref`-inherited uses). One implicit-forwarding case: a definition whose body root is a `$ref` *extends* that base and forwards its own variables into it **by name** (restricted to the variables the base accepts), since the root `$vars` slot is the extending definition's own signature and cannot carry an explicit forward; a variable may be declared solely to pass through to the base. When the base is selected via a *templated* target (`$ref: $defs/tables/base_{{ layer }}`), the concrete base is not known at the upfront signature check, so it unions the accepted variables of every candidate definition whose key matches the pattern — letting a pass-through variable validate against a dynamically-selected base (`_candidate_bases_accepted_vars` in `configs/resolver.py`); at runtime the substituted target resolves to exactly one base. Nested (non-root) `$ref`s still forward explicitly via a `{{ placeholder }}` `$vars` value. `$vars` values are literal strings (a value beginning with `$defs/` is rejected — vars carry values, not references); variable names must be bare identifiers, so an identifier-shaped-but-invalid token like `{{ my-var }}` is rejected rather than passed through literally. Substitution is resolved during `$ref` expansion and stripped before model validation. Missing, unused, incomplete-signature, non-string, `$defs/`-reference-value, malformed-placeholder, and resource-placeholder cases all raise `TemplateVariableError`. Implemented in `configs/variables.py` + `configs/resolver.py`; see the [feature proposal](https://github.com/liamperritt/uc-declarative-abac/issues/18) for the full spec.
...
- Comments — put all substantial explanation (the *why*, edge cases, rationale, invariants, multi-sentence context) in the function/module **docstring**, not in inline comments. Inline `#` comments are reserved for **small, single-line** notes on a specific statement. If an inline comment needs more than one line, it belongs in the docstring; where a statement needs context that lives in the docstring, keep the inline comment to a one-line pointer (e.g. `# ... (see docstring)`)
...
`README.md` is **customer-facing**. Document observable behaviour and configuration, not
internal implementation or performance tuning — customers don't care how the state fetch is
optimised, only what the feature does and how to configure it. Keep internal reasoning
(query shapes, fetch/merge mechanics, why-it's-fast) in code comments/docstrings and
`AGENTS.md`, never in `README.md`. Don't add prose to `README.md` when it's already correct —
adding fat that restates or over-explains is a regression, not an improvement.
