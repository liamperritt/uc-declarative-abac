...
def test_orchestrator_compiles_only_declared_domains_when_creation_scope_active(
    ...
    fetch_actual_domains = WorkspaceHelper.fetch_actual_domains
    with patch.object(
        WorkspaceHelper,
...
def test_orchestrator_creates_only_domains_matching_creation_scopes(
    tmp_yaml_dir, mock_workspace_client, monkeypatch
):
    config = {
        "resources": {
    ...
    result = run(
        ...
        workspace_client=mock_workspace_client,
        warehouse_id="test-warehouse-id",
        domain_creation_scopes="finance*",
        dry_run=True,
    )
...
def test_orchestrator_does_not_create_declared_domain_when_only_management_scope_active(
    tmp_yaml_dir, mock_workspace_client, monkeypatch
):
    """Management scope gates updates only: with no creation scope, a declared
    domain absent from the workspace is never created."""
    config = {
        "resources": {
            "governed_tags": {"finance": {"description": "Financial data"}},
...
def test_orchestrator_creation_and_management_scopes_gate_domains_independently(
    tmp_yaml_dir, mock_workspace_client, monkeypatch
):
    """Creation scope gates creates and management scope gates updates, acting
    independently: a missing domain in the creation scope is created while a
    differing existing domain in the management scope is updated."""
    config = {
        "resources": {
            "governed_tags": {
                "finance": {"description": "Financial data"},
    ...
    with patch.object(
        ...
        result = run(
            ...
            warehouse_id="test-warehouse-id",
            domain_creation_scopes="finance*",
            domain_management_scopes="operations*",
            dry_run=True,
        )
...
def test_orchestrator_creates_domain_from_preexisting_governed_tag(
    tmp_yaml_dir, mock_workspace_client, monkeypatch
):
    config = {
        "resources": {
...
def test_orchestrator_does_not_create_undeclared_domain_for_actual_only_governed_tag_selected_for_deletion(
    ...
    assert Domain(tag_key="finance") not in result.domain_diff.to_create


def test_orchestrator_updates_domain_with_configured_description_when_fetched_tag_is_stale(
    tmp_yaml_dir, mock_workspace_client, monkeypatch
):
    config = {
        "resources": {
...
def test_orchestrator_skips_domain_fetch_when_management_is_disabled(
    tmp_yaml_dir, mock_workspace_client, monkeypatch
):
    config = {
        "resources": {
...
def test_orchestrator_runs_tags_workflow_end_to_end(
    ...
    assert len(result.tag_diff.to_add) > 0, (
        "Expected tags to be added for the tagged catalog"
    )
...
def test_orchestrator_runs_privileges_workflow_end_to_end(
    ...
    assert len(grant_stmts) > 0, f"Expected GRANT SQL to be executed, got: {executed}"
...
def test_orchestrator_threads_max_parallel_changes_to_each_executor(
    ...
    with (
        patch("uc_declarative_abac.orchestrator.execute_tag_diff") as m_tag,
        ...
        patch("uc_declarative_abac.orchestrator.execute_governed_tag_diff"),
    ):
        _run_all_enabled(
            config_dir=root,
            workspace_client=mock_workspace_client,
...
def test_orchestrator_runs_policies_workflow_end_to_end(
    ...
    create_stmts = [
        s for s in mock_workspace_client.executed_sql if "CREATE POLICY" in s.upper()
    ]
    assert len(create_stmts) == 1, (
        f"Expected CREATE POLICY SQL, got: {mock_workspace_client.executed_sql}"
    )
...
def test_orchestrator_policies_workflow_is_idempotent(
    ...
    fake_policy.match_columns = [
        MagicMock(alias="c_pii", condition="has_tag_value('pii', 'email')")
    ]
...
def test_orchestrator_deletes_stale_policy_when_deletion_enabled_and_forced(
    ...
    drop_stmts = [
        s for s in mock_workspace_client.executed_sql if "DROP POLICY" in s.upper()
    ]
    assert len(drop_stmts) == 1, (
        f"Expected DROP POLICY SQL, got: {mock_workspace_client.executed_sql}"
    )
...
def test_orchestrator_is_idempotent_for_service_principal_across_display_name_and_app_id():
    ...
    assert result.tag_diff.to_add == set(), (
        f"Expected no tag changes, got to_add: {result.tag_diff.to_add}"
    )
    assert result.tag_diff.to_update == set()
    assert result.tag_diff.to_remove == set()
    assert result.privilege_diff.to_grant == set(), (
        f"Expected no grants, got: {result.privilege_diff.to_grant}"
    )
    assert result.privilege_diff.to_revoke == set(), (
        f"Expected no revokes, got: {result.privilege_diff.to_revoke}"
    )
    assert result.policy_diff.to_create == set(), (
        f"Expected no policy creates, got: {result.policy_diff.to_create}"
    )
    assert result.policy_diff.to_replace == set(), (
        f"Expected no policy replaces, got: {result.policy_diff.to_replace}"
    )
    ...
    assert mutation_stmts == [], (
        f"Expected no mutation SQL on second run, got: {mutation_stmts}"
    )
...
def _seed_actual_state_for_sp_idempotency(
    mock_workspace_client: MagicMock, sp_app_id: str
) -> None:
    ...
    def _rows_for_sql(sql: str) -> list[list[str]]:
        ...
        if (
            "catalog_owner" in lower
            or "schema_owner" in lower
            or "routine_owner" in lower
        ):
            return [
        ...
        if "catalog_tags" in lower or "schema_tags" in lower:
            return [
                ["CATALOG", "my_catalog", '[{"tag_name":"team","tag_value":"sales"}]']
            ]
        # Privileges query
...
def test_orchestrator_does_not_fetch_actual_tags_when_both_tag_and_privilege_management_disabled(
    ...
    tag_queries = [
        s for s in mock_workspace_client.executed_sql if "_tags" in s.lower()
    ]
    assert tag_queries == [], (
...
def test_orchestrator_does_not_fetch_actual_privileges_when_privilege_management_disabled(
    ...
    privilege_queries = [
        s for s in mock_workspace_client.executed_sql if "_privileges" in s.lower()
    ]
...
def test_orchestrator_privilege_filter_scopes_grants_to_listed_catalog_only(
    ...
    grant_stmts = [
        s for s in mock_workspace_client.executed_sql if s.upper().startswith("GRANT")
    ]
...
def test_orchestrator_taggable_management_filter_scopes_attribute_updates_to_listed_catalog_only(
    ...
    assert "cat_b" not in full_names, (
        f"Expected no cat_b owner update, got: {full_names}"
    )
...
def test_orchestrator_taggable_creation_filter_still_creates_functions_in_out_of_scope_catalogs(
    ...
    create_func_stmts = [
        s
        for s in mock_workspace_client.executed_sql
        if "FUNCTION" in s.upper() and "CREATE" in s.upper()
    ]
...
def test_orchestrator_tag_filter_uses_actual_tags_for_out_of_scope_catalogs_when_grants_match(
    ...
    cat_b_grants = [
        ...
        if p.securable_full_name.startswith("cat_b")
    ]
    assert cat_b_grants, (
...
def test_orchestrator_creation_filter_qualified_schema_does_not_create_parent_catalog(
    ...
    cat_a_errors = [e for e in nonexistent if e.exception.full_name == "cat_a"]
    assert cat_a_errors, (
        "Expected NonexistentSecurableError for out-of-scope parent catalog cat_a"
    )
...
def test_orchestrator_deletes_governed_tag_when_flag_and_force_enabled(
    ...
    mock_workspace_client.tag_policies.delete_tag_policy.assert_called_once_with(
        "legacy_pii"
    )
...
def test_orchestrator_raises_ungoverned_tag_error_when_grant_policy_references_ungoverned_tag(
    ...
    ungoverned_errors = [
        e for e in exc_info.value.errors if isinstance(e.exception, UngovernedTagError)
    ]
            ...
            "catalogs": {
                "my_catalog": {
                    ...
                    "schemas": [
                        {
                            "name": "sales",
                            "rfa_destinations": ["https://hook.example.com"],
                        },
                    ],
                }
            }
...
def test_orchestrator_rename_does_not_diff_actual_grants_referencing_old_name(
    ...
    def _references_group(sp) -> bool:
        return (
            sp.principal.name in group_names or sp.principal.identifier in group_names
        )

    offending_grants = [
        sp for sp in result.privilege_diff.to_grant if _references_group(sp)
    ]
    offending_revokes = [
        sp for sp in result.privilege_diff.to_revoke if _references_group(sp)
    ]
